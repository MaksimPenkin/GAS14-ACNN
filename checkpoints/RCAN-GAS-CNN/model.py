        ...
        with tf.compat.v1.variable_scope(scope):
            ...
            for i in range(self.conv_blocks_num):
                if (i > self.conv_blocks_num//2):
                    self.H_conv[finish_block] = residual_channel_attention_block(self.H_conv[finish_block] + self.H_conv[j], self.feature_size, ksize=self.cnn_size, 
                        regularizator=slim.l2_regularizer(scale=self.reg_scale), reuse=var_reuse, trainable=var_trainable, scope='RCAN{0}'.format(i), is_training=self.is_training)
                    j=j-1
                else:
                    self.H_conv[i+1] = residual_channel_attention_block(self.H_conv[i], self.feature_size, ksize=self.cnn_size, 
                        regularizator=slim.l2_regularizer(scale=self.reg_scale), reuse=var_reuse, trainable=var_trainable, scope='RCAN{0}'.format(i), is_training=self.is_training)
